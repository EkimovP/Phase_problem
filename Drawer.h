...
public:
	...
	void DrawGrid(vector<double> y_first_graph, int typefunc, vector<double> x_first_graph, int N)
	{
		...
		data_y_max = *max_element(y_first_graph.begin(), y_first_graph.end());
		...
		{
			CString str;
			str.Format(L"%.1f", data_x_min + i * (data_x_max) / (grid_size / 2));
	...
	void DrawPlot(vector<double>& y_first_graph, int typefunc, vector<double>& x_first_graph, int N)
	{
		...
		CPen data_pen2(PS_SOLID, 1, RGB(0, 0, 0));
		CPen data_pen3(PS_SOLID, 1, RGB(0, 0, 255));
